        ...
        Box(
            ...
            when (val state = uiState) {
                ...
                is AcademicTaskUiState.Error -> {
                    // Contenedor de recuperación formal (Desafío Extra)
...
fun AcademicTaskCreateScreen(viewModel: AcademicTaskViewModel) {
    // Suscripción al estado de la UI para detectar si el caso de uso arrojó un error
    val uiState by viewModel.uiState.collectAsState()
    ...
    Scaffold { paddingValues ->
        Column(
            ...
            // Si el estado actual es Error, renderizamos el contenedor de recuperación (Desafío Extra)
            if (uiState is AcademicTaskUiState.Error) {
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Text(
                        text = (uiState as AcademicTaskUiState.Error).message,
                        color = MaterialTheme.colorScheme.error,
                        style = MaterialTheme.typography.bodyLarge
                    )
                    Spacer(modifier = Modifier.height(16.dp))
                    Button(onClick = { viewModel.onNavigateToCreate() }) { // Limpia el error y permite reintentar
                        Text("Reintentar")
                    }
                }
            } else {
                // Si todo está normal, muestra el formulario de inserción estándar
                Text(
